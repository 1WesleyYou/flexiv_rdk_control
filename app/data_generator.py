...
# 文件路径
file_path = '../simulate_datasets/offset_order.txt'
time_span = 40  # 假设这里是 100 ms 时间延迟，具体的需要按收包实际时间差考虑
total_time = 250
last_data = [0, -40 * np.pi / 180, 0, 90 * np.pi / 180, 0, 40 / 180 * np.pi, 0]
data_a4_last = []
data_a4_angle = []
            ...
            try:
                ...
                for i in range(time_span):
                    ...
                    if data['joint_positions'][0]:
                        ...
                        local_datas = {'joint_positions': last_data, 'joint_velocities': [0, 0, 0, 0, 0, 0, 0],
                                       'joint_efforts': is_gripper_closed}
                    json_string = json.dumps(local_datas)  # 将解析后的JSON重新编码为字符串
                    data_a4_last.append(local_data[0] * 180 / np.pi)
                ...
                if time_cnt >= total_time:
                    break

            except json.JSONDecodeError as e:
                print(f"JSON解析错误: {e}")
        t = np.linspace(0, total_time, int(total_time / 0.001) + 100)
