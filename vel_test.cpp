...
void printDescription() {
  std::cout << "This tutorial runs real-time joint position control to hold or "
               "sine-sweep all "
               "robot joints."
            << std::endl
            << std::endl;
  ...
  try {
    ...
    if (motionType == "hold") {
      ...
      for (size_t i = 0; i < robotDOF; ++i) {
        targetPos[i] = initPos[i] + k_sineAmp * sin(2 * M_PI * k_sineFreq *
                                                    loopCounter * k_loopPeriod);
      }
    } else {
      throw flexiv::InputException("periodicTask: unknown motion type. "
                                   "Accepted motion types: hold, sine-sweep");
    }
...
int main(int argc, char *argv[]) {
  ...
  try {
    ...
    scheduler.addTask(std::bind(periodicTask, std::ref(robot), std::ref(log),
                                std::ref(robotStates), std::ref(motionType),
                                std::ref(initPos)),
    ...
    // Wait a bit for any last-second robot log message to arrive and get
    // printed
    std::this_thread::sleep_for(std::chrono::seconds(2));
